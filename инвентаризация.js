...
function invImpRefreshTargets(){
  ...
  db.collection('iz_inventory_sessions').where('shopName','==',shop).get({source:'server'}).then(function(snap){
    ...
    Promise.all(matches.map(function(x){
      ...
      // Раньше тут по умолчанию сразу выставлялся выбор «дозаписать в найденную» — и если открыть
      // «Загрузить список» второй раз для того же магазина/даты/вида (например, засомневавшись,
      // сохранилось ли в первый раз, и повторив всю загрузку заново) список молча задваивался:
      // количество каждой позиции складывалось с уже сохранённым. Теперь по умолчанию — «Новая»,
      // дозапись в найденную нужно выбрать осознанно самому, а не она подставляется автоматически.
      sel.innerHTML = opts; sel.value = '';
      if(hint) hint.textContent = '⚠️ Для этого магазина/даты/вида товара уже есть начатая инвентаризация. Если сейчас вносите СЛЕДУЮЩУЮ страницу той же — выберите её выше («📥 Добавить в начатую»), иначе количество каждой позиции сложится дважды. Если это отдельное новое внесение — оставьте «Новая».';
    });
