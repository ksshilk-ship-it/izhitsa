...
function saveShifts(shifts){
  ...
  try{
    ...
    try{
      ...
      showToast('⚠️ Лимит хранилища браузера переполнен (5-10 МБ на сайт) — эта запись не закешируется локально, но сохранится в облаке');
      return true;
    }
  }
}
    ...
    db.collection('iz_shifts').get({source:'server'}).then(function(snap){
      ...
      if(status){
        status.textContent = wasPruned
          ? '⚠️ Подтянуто '+snap.size+' смен, но лимит хранилища браузера (5-10 МБ на сайт — не зависит от места на диске) не вместил все недавние смены локально. Показываться они всё равно будут — просто подтянутся из облака заново при следующем открытии, а не из локального кэша.'
...
function _renderShiftView(){
  ...
  var woodWoTotal = woodWo.reduce(function(s,w){return s+(w.amount||w.amt||0);},0) + jWoWoodTotal;
  var drWoTotal = drWo.reduce(function(s,w){return s+(w.amount||w.amt||0);},0) + jWoDrTotal;
