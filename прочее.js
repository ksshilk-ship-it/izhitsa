      ...
      '<div id="rbBody_'+book.id+'" style="display:none;padding:10px;border-top:1px solid #2e2e3e">'+
        ...
        (book.key==='iz_goods_dr'
          ...
          ? '<div style="display:flex;flex-direction:column;gap:10px;margin-top:8px">'+
              '<div style="display:flex;flex-direction:column;gap:6px;padding:8px;background:#13131a;border-radius:8px">'+
                '<div style="font-size:10px;color:#8888aa">📋 Просто наименование — без породы, цены и артикула. Артикул присвоится системой при приёмке на магазине, либо изделие уже с артикулом из мастерской</div>'+
                '<div style="display:flex;gap:8px">'+
                  '<input class="fi" id="rbNew_'+book.id+'" placeholder="Новое наименование" autocomplete="off" style="flex:1;margin:0">'+
              ...
              '</div>'+
              '<div style="display:flex;flex-direction:column;gap:6px;padding:8px;background:#13131a;border-radius:8px">'+
                '<div style="font-size:10px;color:#8888aa">🔢 Позиция с артикулом вручную — для переноса старых записей (аудит наименований)</div>'+
                '<input class="fi" id="rbWdName_'+book.id+'" placeholder="Наименование" autocomplete="off" style="margin:0">'+
                '<input class="fi" id="rbWdSpecies_'+book.id+'" placeholder="Порода" autocomplete="off" style="margin:0">'+
                '<div style="display:flex;gap:8px">'+
                  '<input class="fi" id="rbWdArticle_'+book.id+'" placeholder="Артикул" autocomplete="off" style="flex:1;margin:0">'+
...
function renderGoodsCatalogGrouped(bookId, key){
  ...
  var allCatsForList = [];
  items.forEach(function(it){ var cat=(it.category||'').trim(); if(cat && allCatsForList.indexOf(cat)<0) allCatsForList.push(cat); });
  function renderGroup(subItems, richTemplate, sectionKey){
    var groups = {};
    ...
    catNames.forEach(function(cat){
      ...
      var gid = bookId+'__'+sectionKey+'__'+cat;
      var open = window._rbCatOpen[gid]===true;
      out += '<div onclick="toggleGoodsCategoryGroup(this)" data-book="'+bookId+'" data-key="'+key+'" data-cat="'+cat.replace(/"/g,'&quot;')+'" data-section="'+sectionKey+'" style="display:flex;justify-content:space-between;align-items:center;cursor:pointer;padding:8px 6px;margin:10px 0 4px;background:#1a1a22;border-radius:8px">'+
      ...
      if(open){
        list.forEach(function(item){
          ...
          var name = item.name||item;
          if(richTemplate){
            out += '<div style="padding:8px;background:#13131a;border-radius:8px;margin-bottom:6px">'+
            ...
            out += '<div style="display:flex;align-items:center;gap:6px;padding:8px;background:#13131a;border-radius:8px;margin-bottom:6px">'+
    ...
    var body = species.map(function(sp){
      ...
      var rowsHtml = rows.map(function(item){
        ...
        return '<div style="display:flex;align-items:center;gap:4px;padding:4px 10px 4px 18px">'+
          '<input type="text" value="'+(item.article||'').replace(/"/g,'&quot;')+'" placeholder="№" onchange="setRefbookItemArticle(\''+bookId+'\','+realIdx+',\''+key+'\',this.value)" style="width:100px;flex-shrink:0;background:#22222e;border:1px solid #2e2e3e;border-radius:6px;color:#f0f0f8;font-size:11px;padding:5px 5px">'+
          '<input type="text" value="'+(item.name||'').replace(/"/g,'&quot;')+'" placeholder="Наименование" onchange="setRefbookItemName(\''+bookId+'\','+realIdx+',\''+key+'\',this.value)" style="flex:1;min-width:36px;background:#22222e;border:1px solid #2e2e3e;border-radius:6px;color:#f0f0f8;font-size:11px;padding:5px 6px">'+
          '<input type="number" value="'+(item.price!=null?item.price:'')+'" placeholder="Цена" onchange="setRefbookItemPrice(\''+bookId+'\','+realIdx+',\''+key+'\',this.value)" style="width:48px;flex-shrink:0;background:#22222e;border:1px solid #2e2e3e;border-radius:6px;color:#f0f0f8;font-size:11px;padding:5px 5px">'+
          '<input type="text" value="'+(item.species||'').replace(/"/g,'&quot;')+'" placeholder="Порода" onchange="setRefbookItemSpecies(\''+bookId+'\','+realIdx+',\''+key+'\',this.value)" style="width:56px;flex-shrink:0;background:#22222e;border:1px solid #2e2e3e;border-radius:6px;color:#f0f0f8;font-size:11px;padding:5px 5px">'+
...
// Раздел «с артикулом вручную» — для переноса старых записей (аудит наименований): артикул
...
function addWoodGoodsVariant(bookId, key){
  ...
  var artEl = document.getElementById('rbWdArticle_'+bookId);
...
function toggleGoodsCategoryGroup(headerEl){
  ...
  var section = headerEl.getAttribute('data-section') || 'main';
  var gid = bookId+'__'+section+'__'+cat;
  window._rbCatOpen = window._rbCatOpen || {};
...
function runArticleAudit(){
  ...
  setTimeout(function(){
    ...
    var noCatalogByName = {}; // name -> {count, prices:{price:{count,refs:[]}}}
    occurrences.forEach(function(o){
      ...
      if(!variants.length){
        ...
        if(!noCatalogByName[o.name].prices[pr]) noCatalogByName[o.name].prices[pr] = {count:0, refs:[]};
        noCatalogByName[o.name].prices[pr].count++;
        noCatalogByName[o.name].prices[pr].refs.push(o.ref);
        return;
      }
      var article = null;
    ...
    _artNoCatalog = Object.keys(noCatalogByName).map(function(n){
      ...
      var priceList = Object.keys(entry.prices).map(function(p){
        return {price:parseFloat(p), count:entry.prices[p].count, refs:entry.prices[p].refs};
      }).sort(function(a,b){ return b.count-a.count; });
